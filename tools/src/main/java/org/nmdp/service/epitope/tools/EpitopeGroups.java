...
/**
 * Epitope groups command line tool.
 */
public final class EpitopeGroups extends AbstractEpitopeTool {
    private final String alleles;
    private final String alleleUris;
    private final String groups;
    static final String USAGE = "epitope-groups -u " + DEFAULT_ENDPOINT_URL + " -g 2";

    public EpitopeGroups(final String endpointUrl, final List<String> alleles, final List<URL> alleleUris, final List<Integer> groups, final File inputFile, final File outputFile) {
    ...
    public Integer call() {
        ...
        try {
            ...
            write(getEpitopeService().getGroups(null, alleles, null, alleleUris, null, groups), writer);
            ...
            while (reader.ready()) {
                ...
                write(getEpitopeService().getGroups(null, null, null, null, null, line.trim()), writer);
            }
    ...
    static void write(final List<GroupView> results, final PrintWriter writer) throws IOException {
        for (GroupView result : results) {
            Integer group = result.getGroup();
            for (String allele : result.getAlleleList()) {
    ...
    public static void main(final String args[]) {
        ...
        FileArgument inputFile = new FileArgument("i", "input-file", "input file of groups, default stdin", false);
        FileArgument outputFile = new FileArgument("o", "output-file", "output file of tab-delimited group views, default stdout", false);

        ArgumentList arguments = new ArgumentList(about, help, endpointUrl, alleles, alleleUris, groups, inputFile, outputFile);
        CommandLine commandLine = new CommandLine(args);

        EpitopeGroups epitopeGroups = null;
        try
        {
            ...
            epitopeGroups = new EpitopeGroups(endpointUrl.getValue(DEFAULT_ENDPOINT_URL), alleles.getValue(), alleleUris.getValue(), groups.getValue(), inputFile.getValue(), outputFile.getValue());
        }
        ...
        try {
            System.exit(epitopeGroups.call());
        }
        catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
